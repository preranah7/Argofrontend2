...
// src/components/ChatInterface.jsx - Updated to use BotResponseCard analytics
import React, {
  useState,
  useRef,
  useEffect,
...
class ChatErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex items-center justify-center h-64 text-center p-8 bg-gray-900">
          <div className="text-gray-300 bg-gray-800 rounded-xl p-6 border border-gray-700">
            <h3 className="text-lg font-semibold mb-2 text-white">Something went wrong</h3>
            <p className="mb-4 text-gray-400">The chat encountered an error.</p>
            <button 
              onClick={() => this.setState({ hasError: false })}
              className="px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-500 mr-2 transition-colors"
            >
              Try Again
            </button>
            <button 
...
// Simplified Message Item - Charts now handled in BotResponseCard
  ...
  return (
    <div className="px-4 sm:px-6 mb-3">
      <div className={`flex ${isUser ? "justify-end" : "justify-start"} items-start gap-2`}>
        {/* Bot Avatar */}
        {isBot && (
          <div className="flex-shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center shadow-lg mt-1">
        ...
        <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-5xl`}>
          {isBot && message.response ? (
            <div className="w-full">
              <div className="rounded-2xl border-l-4 border-cyan-400/70 bg-gray-800/95 backdrop-blur-sm shadow-xl ring-1 ring-gray-700/50">
            ...
            <div
              className={
                isUser
                  ? "max-w-sm px-4 py-2.5 rounded-2xl bg-gradient-to-br from-slate-600 to-slate-700 text-white shadow-lg text-sm border border-slate-500/30"
                  : "max-w-3xl px-6 py-4 rounded-2xl bg-gray-800/90 border border-gray-600/50 text-gray-100 backdrop-blur-sm shadow-xl"
              }
              role="article"
            ...
            >
              ...
              <div className={`text-xs opacity-75 mt-2 flex items-center ${isUser ? 'justify-between' : 'justify-between'}`}>
        ...
        {/* User Avatar */}
        {isUser && (
          <div className="flex-shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-slate-600 to-slate-700 flex items-center justify-center shadow-lg mt-1 ring-2 ring-slate-500/30">
    ...
    return (
      <div className="w-full px-4 sm:px-6">
        <div className="mx-auto max-w-4xl">
          ...
          <AnimatePresence>
            {draftSaved && (
              <motion.div
                ...
                className="mb-3 text-xs text-emerald-400 flex items-center justify-center bg-emerald-900/20 px-4 py-2 rounded-xl border border-emerald-500/30 backdrop-blur-sm"
              >
          ...
          <AnimatePresence>
            {isTyping && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="mb-3 text-sm text-gray-400 flex items-center justify-center"
                role="status"
                aria-live="polite"
              >
          ...
          <div
            className="
              flex items-end gap-2 w-full rounded-2xl
          ...
          >
            ...
            <button
              ...
              className="p-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-gray-300 hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/40"
            >
            ...
            <div className="relative z-50" ref={modelMenuRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-2 text-xs sm:text-sm rounded-xl bg-neutral-800 hover:bg-neutral-700 text-gray-200 hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/40"
              >
                <FaRobot className="text-cyan-400 text-sm sm:text-base" />
                <span className="font-medium">{selectedModel}</span>
                <FaChevronDown className={`text-gray-400 text-xs transition-transform duration-200 ${showModelMenu ? 'rotate-180' : ''}`} />
              </button>

              {showModelMenu && (
                <div className="absolute bottom-full mb-2 right-0 w-48 bg-neutral-900 border border-neutral-600 rounded-xl shadow-2xl z-50 py-2">
                  <div className="px-3 py-1 text-xs text-gray-400 font-semibold border-b border-neutral-700 mb-1">
                    Select Model
                  </div>
                  {["GPT", "QWEN", "LLaMA", "Mistral"].map((model) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm bg-neutral-900 hover:bg-neutral-700 transition-colors ${
            ...
            <div className="flex-1 min-w-0">
              <textarea
                ...
                className="
                  ...
                  px-2 sm:px-3 py-2 leading-6
                  max-h-[14rem] overflow-y-auto thin-scroll
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/40 ${
                isRecording ? "bg-rose-600 hover:bg-rose-500 text-white" : "bg-neutral-800 hover:bg-neutral-700 text-gray-300"
              }`}
            >
              {isRecording ? <FaMicrophoneSlash className="text-base sm:text-lg" /> : <FaMicrophone className="text-base sm:text-lg" />}
            </button>
            ...
            <button
              ...
              className="p-2.5 sm:p-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 disabled:bg-neutral-700 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/40 shadow-md disabled:cursor-not-allowed"
            >
              {loading ? (
      ...
      <div className="flex flex-col h-screen bg-gray-950 text-white">
        <AnimatePresence mode="wait">
          {showLanding ? (
            ...
            >
              <div className="flex items-center justify-center w-20 h-20 mb-6 rounded-2xl bg-gray-800 border border-gray-700 shadow-2xl">
                <FaRobot className="text-3xl text-cyan-400" />
              </div>
              <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">ARGO FloatChat</h1>
            ...
            >
              <div className="flex-1 overflow-y-auto px-0 pt-2 pb-4 sm:pb-6">
              ...
              <div className="border-t border-gray-700/50 mt-1 mb-3 sm:mb-5">
                <Composer
                  ref={textareaRef}
                  displayValue={displayValue}
                  onChangeBaseInput={(v) => {
